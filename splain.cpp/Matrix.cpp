...
vector<vector <double>> getMat(const char* fileName) {
	...
	for (int i = k,j=0,fo,foo,kk=k; i < kk+m-2; ++i) { //заполняем условия на промежуточных границах 
	...
	mat[k][m-2] = 1;						   //заполняем условия поведения фукнции 
	mat[k][m-1] = 3 * vecx[0];				  //на внешних границах через производные
	++k;									 //второго порядка  S''[0][x0]=0, S''[n-1][xn]=0 ..
	mat[k][n-2] = 1;						//ps последие уравнения 
